...
ORG=nobregaX-dumps #your GitHub org name
...
if [[ -n $GIT_OAUTH_TOKEN ]]; then
    ...
    if [[ -z "$(git config --get user.email)" ]]; then
        git config user.email otavio.nobregadas@gmail.com
    fi
    if [[ -z "$(git config --get user.name)" ]]; then
        git config user.name nobregaX
    fi
    ...
    allowlist=(
        ...
        "MiuiCamera.apk"
        # Allow isp_dts.img to be compressed
        "i\sp_dt\s.img"
        "isp_dts.img"
    )
    compressed_files=()
